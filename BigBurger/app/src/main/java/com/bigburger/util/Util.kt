...
fun distanceBetweenPoints(latA: Double, lngA: Double, latB: Double, lngB: Double): Double {
    val distance: Double
    ...
    distance = locationA.distanceTo(locationB).toDouble()   // in meters
    Log.d("Distance", distance.toString() + " between point 1: " + latA + "," + lngA + " point 2: " + latB + "," + lngB)

    return distance
}
fun getDistanceString(context: Context, distanceInMeters: Double): String {
   ...
   if (distanceInMeters == -1.0) {
        ...
        distance = String.format("%.1fm", distanceInMeters)
    }
    return distance
}
